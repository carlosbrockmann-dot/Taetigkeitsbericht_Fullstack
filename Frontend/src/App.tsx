...
export default function App() {
  const [session] = useState<Session | null>(() => bootstrapSessionFromUrl())
  ...
  useEffect(() => {
    ...
    if (session?.mandantId != null) {
      params.set('mandantId', String(session.mandantId))
    }
    const next = `${window.location.pathname}?${params.toString()}`
    window.history.replaceState(null, '', next)
  }, [jahr, monat, session?.mandantId])

  if (!session?.token) {
    return (
      <div className="page">
        <header className="header">
          <h1>Tätigkeitsbericht – Online</h1>
        </header>
  ...
  return (
    <div className="page">
      <header className="header">
        <h1>Tätigkeitsbericht – Online</h1>
        <p className="sub">
          Mandant {session.mandantId ?? '—'} · nur Ansicht (Backend)
        </p>
      </header>
              ...
              <tbody>
                {kalenderzeilen.map((zeile) => (
                  ...
                  >
                    ...
                    <td>{kategorieLabel(zeile.eintrag?.kategorie)}</td>
