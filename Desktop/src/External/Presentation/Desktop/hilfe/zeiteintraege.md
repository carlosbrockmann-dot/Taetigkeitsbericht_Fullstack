...
| **Online ansehen**             | Meldet sich bei Bedarf am Backend an (bestehendes JWT wird zuvor geprüft; bei ungültig/abgelaufen erneuter Login) und öffnet die React-Online-Ansicht im **Standardbrowser** (aktueller Monat und Mandant, gültiges JWT, Login-Benutzername, Mandantenliste aus `mandanten.toml` als JSON im URL-Hash). Voraussetzung: Frontend läuft (`npm run dev` unter `Frontend/`). |
