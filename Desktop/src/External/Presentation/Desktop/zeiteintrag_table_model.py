...
from PySide6.QtCore import QAbstractTableModel, QModelIndex, QPersistentModelIndex, QPointF, Qt, QRect
...
# Gleicher Grauton wie Wochenendzeilen (siehe data(), Qt.ItemDataRole.BackgroundRole).
...
_FEIERTAG_STERN_ICON_CACHE: QIcon | None = None
...
def feiertag_stern_icon() -> QIcon:
    global _FEIERTAG_STERN_ICON_CACHE
    if _FEIERTAG_STERN_ICON_CACHE is not None:
        return _FEIERTAG_STERN_ICON_CACHE
    ...
    maler.end()
    _FEIERTAG_STERN_ICON_CACHE = QIcon(pm)
    return _FEIERTAG_STERN_ICON_CACHE
...
class ZeiteintragTableModel(QAbstractTableModel):
    ...
    HEADER_TOOLTIPS = [
        ...
        "Geleistete Stunden",
        "Soll nach Stundenplan",
        "Soll nach Vertrag",
        "Kategorie: U/K nur an geregelten Vertragstagen, sonst leer",
    ...
    def data(
        ...
        col = index.column()
        if role == Qt.ItemDataRole.TextAlignmentRole and col in ZeiteintragSpalte.STATUS_KENNZEICHEN | {
            ZeiteintragSpalte.TAG_EXCEL,
            ZeiteintragSpalte.KATEGORIE,
        }:
        ...
        match col:
            ...
            case ZeiteintragSpalte.KATEGORIE:
    ...
    def setData(  # noqa: N802
        ...
        if col == ZeiteintragSpalte.DATUM:
            ...
            if self._rows:
                ...
                self.dataChanged.emit(v0, v1, [Qt.ItemDataRole.DisplayRole, Qt.ItemDataRole.EditRole])
            return True
        ...
        if col in ZeiteintragSpalte.ZEITFELDER:
            ...
            self.dataChanged.emit(
                left, right, [Qt.ItemDataRole.DisplayRole, Qt.ItemDataRole.EditRole, Qt.ItemDataRole.BackgroundRole]
            )
            return True

        self.dataChanged.emit(
            index, index, [Qt.ItemDataRole.DisplayRole, Qt.ItemDataRole.EditRole, Qt.ItemDataRole.BackgroundRole]
        )
        return True
    ...
    def feiertag_darstellung_aktualisieren(self) -> None:
        ...
        self.dataChanged.emit(
            top_left,
            bottom_right,
            [Qt.ItemDataRole.DisplayRole, Qt.ItemDataRole.BackgroundRole, Qt.ItemDataRole.ToolTipRole, Qt.ItemDataRole.DecorationRole],
        )
